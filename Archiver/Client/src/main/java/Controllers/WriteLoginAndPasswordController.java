...
public class WriteLoginAndPasswordController {
     ...
     * @param mainController zwraca kontroler do glownego okna programu (MainScreena)
     */
    ...
    /**
     * Metoda obslugujaca zamkniecie aplikacji za pomocą przycisku closeApplication
     */
    @FXML
     ...
     * @return zwraca ilosc wystapien danego znaku w napisie
     */
    public static int findAndCountCharinString(String inscription, char character){
        ...
        if(correctConnect){
            ...
            try {
                if (LoginTextField.getText().trim().isEmpty() || PasswordTextField.getText().trim().isEmpty()){
                    Alert alert = new Alert(Alert.AlertType.NONE, "Należy wpisać login i hasło!", ButtonType.OK);
                    alert.showAndWait();
                }else {
                    ...
                    stringBuilder.append("LOGIN:" + LoginTextField.getText().trim() + "%" + PasswordTextField.getText().trim());
                    ...
                    if (GetInputStream.getAnswear(messagefromserver)[0].equals("200")) {
                        ...
                        Alert alert = new Alert(Alert.AlertType.NONE, "Podany użytkownik jest już zalogowany", ButtonType.OK);
                        alert.showAndWait();
                    } else if (GetInputStream.getAnswear(messagefromserver)[0].equals("400")) {
                        Alert alert = new Alert(Alert.AlertType.NONE, "Nieprawidłowy login lub hasło", ButtonType.OK);
                        alert.showAndWait();
                    } else {
                    }
                }
